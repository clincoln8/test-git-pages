...
test('testing setDCID w/ merge', async () => {
  ...
  const remoteNode = Node.getNode('remoteId');
  remoteNode.setDCID('remoteId');

  const targetNode = Node.getNode('targetId');
  const srcNode = Node.getNode('srcId');
  ...
  const localNode = Node.getNode('localId');
...
test('testing merge duplicate assertions', async () => {
  ...
  const localNode = Node.getNode('localNode');
  const localValNode = Node.getNode('localId');
  const remoteNode = Node.getNode('remoteId');
  remoteNode.setDCID('remoteId');
...
test('testing getRef', () => {
  ...
  const node = Node.getNode('localId');
