...
test('testing setCurNode', () => {
  ...
  const remoteNameSpaceExpectedNode =
      new Node('dcid:'+remoteNameSpaceParsedVal[0]['ref']);
  ...
  expect(() => mcfParser.setCurNode(localNameSpaceParsedVal))
      .toThrow(new Error('Error with namespace in declaring node (line -1): namespace: l'));
});
...
test('testing setCurNodeDCID', () => {
  ...
  const expectedErrorMsg1 = 'ERROR (line -1): a node can only have one dcid';
  ...
  const expectedErrorMsg2 =
    'ERROR (line -1): dcid property must be a string, not a node reference';
...
test('testing parseLine: ', () => {
  ...
  expect(() => mcfParser.parseLine(testStr4))
      .toThrow(
          new Error('Error, (line -1): ' + testStr4));

  const testStr5 = ' : ';
  expect(() => mcfParser.parseLine(testStr5))
      .toThrow(new Error('Error, (line -1): ' + testStr5.trim()));
